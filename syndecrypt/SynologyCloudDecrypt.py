...
from syndecrypt.__main__ import decrypt as __main__
...
msg = tk.Message(
    top,
    text="This software is provided for free. If you have paid for or have been asked to pay for this software, it is likely not legitimate. Please discontinue using this software and seek help from proper authorities.",
)
...
file = ttk.Radiobutton(root, text="File", variable=item, value=1)

folder = ttk.Radiobutton(root, text="Folder", variable=item, value=2)
            ...
            if filebox.get() != "":
                if outputbox.get() != "":
                    if os.path.dirname(filebox.get()) == outputbox.get():
                        ...
                        return
                    elif outputbox.get().startswith(
                        os.path.abspath(filebox.get()) + os.sep
                    ):
                        tkinter.messagebox.showwarning(
                            "Decryption",
                            "Output folder cannot be inside the encryption folder!",
                        )

                        return
                    elif os.path.basename(filebox.get()) == os.path.basename(
                        outputbox.get()
                    ):
                        if os.path.dirname(filebox.get()) == os.path.dirname(
                            outputbox.get()
                        ):
                            tkinter.messagebox.showwarning(
                                "Decryption",
                    ...
                    try:
                        ...
                        tkinter.messagebox.showwarning(
                            "Decryption",
                            "Failed to decrypt file(s), please raise an issue using the support button located in the about option of the application menu",
                        )
                    else:
                    ...
                    tkinter.messagebox.showwarning(
                        "Decryption", "Please choose output folder!"
                    )
            else:
                tkinter.messagebox.showwarning(
...
def run_tool():
    if method.get() == 1:
        ...
        out = outputbox.get()
        input = filebox.get()
        __main__(["-p", pword, out, input])
    elif method.get() == 2:
        pkey = pkfilebox.get()
        pukey = pufilebox.get()
        out = outputbox.get()
        input = filebox.get()
        __main__(["-k", pkey, pukey, out, input])
...
def about_dialog():
    ...
    copytext = (
        "© " + str(datetime.datetime.now().year) + " - Created by Anojh Thayaparan"
    )
